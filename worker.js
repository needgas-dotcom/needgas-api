  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/orders' && request.method === 'POST') {
      try {
        ...
        // The order reference is generated here, from the database's own row ID,
        // so it can never be duplicated, reset, or reused even under heavy load.
        const newId = insertResult.meta.last_row_id;
        const orderRef = 'NG-' + String(newId).padStart(6, '0');

        await env.DB.prepare('UPDATE orders SET order_ref = ? WHERE id = ?')
          .bind(orderRef, newId)
          .run();

        return new Response(JSON.stringify({ success: true, orderRef: orderRef }), {
